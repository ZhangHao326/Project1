...
std::string TimerQueue::AddFuncAfterDuration(const std::chrono::seconds& time, std::string timer_id, TimerFunc&& func) {
    Timer s;
    s.func_ = std::move(func);
    s.time_point_ = std::chrono::high_resolution_clock::now() + time;
    s.timer_id_ = timer_id;
    {
        std::unique_lock<std::mutex> lock(mutex_);
        auto iter = timer_list_.begin();
        while (iter != timer_list_.end() && (*iter) < s) { ++iter; }
...
bool TimerQueue::RemoveTimer(std::string timer_id) {
